...
void transpose_blocking(int n, int blocksize, int *dst, int *src) {
    //set z to next available blocks
    /*if(n - x < blocksize)
        z += n-x;
    else
        z += blocksize;*/
    ...
    int x=0, y=0;
    //two loops to form block location
    //two loops to transpose the matrix
    ...
    /*
    for(x = 0; x < n; x++) {
        //we increment z by the number of blocks available
        if(z <  < blocksize)
            z += blocksize - y;
        else
            z += blocksize;

        for(y = 0; y < z; y++) { //pass z as limiter for blocksize
            dst[y + x * n] = src[x + y * n];
        }
